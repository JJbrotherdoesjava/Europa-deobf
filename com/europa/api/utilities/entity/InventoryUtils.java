...
implements IMinecraft {
    ...
    public static void switchToSlot(Class<? extends Item> clazz) {
        Class<? extends Item> clazz2;
        if (InventoryUtils.mc.player.getHeldItemMainhand().getItem().getClass().isAssignableFrom(clazz2)) {
            return;
        }
        int slot = InventoryUtils.getHotbarItemSlot(clazz2);
        if (slot == -1) {
            return;
        }
        InventoryUtils.mc.player.inventory.currentItem = slot;
    ...
    public static int getHotbarItemSlot(Class<? extends Item> clazz) {
        int slot = -1;
        for (int i = 0; i < 9; ++i) {
    ...
    public static int getHotbarBlockSlot(Block block) {
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            if (!(item instanceof ItemBlock) || !((ItemBlock)item).getBlock().equals(block2)) continue;
            slot = i;
            break;
        }
        return slot;
    ...
    public static int getHotbarItemSlot(Item item) {
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            if (!selection.equals(item2)) continue;
            slot = i;
            break;
        }
        return slot;
