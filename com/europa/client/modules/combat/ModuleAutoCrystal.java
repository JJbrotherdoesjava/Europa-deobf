...
extends Module {
    ...
    public void doAutoCrystal() {
        block18: {
            block15: {
                block17: {
                    block16: {
                        ...
                        if (!ModuleAutoCrystal.place.getValue()) break block15;
                        if (this.placeTicks++ <= ModuleAutoCrystal.placeDelay.getValue().intValue()) break block15;
                        this.placeTicks = 0;
                        ...
                        for (EntityPlayer player : ModuleAutoCrystal.mc.world.playerEntities) {
                        ...
                        if (this.targetPosition == null) break block16;
                        slot = InventoryUtils.findItem(Items.END_CRYSTAL, 0, 9);
                        lastSlot = ModuleAutoCrystal.mc.player.inventory.currentItem;
                        if (!ModuleAutoCrystal.switchMode.getValue().equals((Object)SwitchModes.None)) {
                            if (slot != -1) {
                                InventoryUtils.switchSlot(slot, ModuleAutoCrystal.switchMode.getValue().equals((Object)SwitchModes.Silent));
                            }
                        }
                        if (ModuleAutoCrystal.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL) ** GOTO lbl-1000
                        if (ModuleAutoCrystal.mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL) ** GOTO lbl-1000
                        if (ModuleAutoCrystal.switchMode.getValue().equals((Object)SwitchModes.Silent)) lbl-1000:
                        // 3 sources
            ...
            if (!ModuleAutoCrystal.breakMode.getValue() || this.breakTicks++ <= ModuleAutoCrystal.breakDelay.getValue().intValue()) break block18;
            this.breakTicks = 0;
            for (EntityPlayer player : ModuleAutoCrystal.mc.world.playerEntities) {
                if (ModuleAutoCrystal.mc.player.getDistanceSq((Entity)player) > (double)MathUtils.square(ModuleAutoCrystal.targetRange.getValue().floatValue()) || player == ModuleAutoCrystal.mc.player || Europa.FRIEND_MANAGER.isFriend(player.getName()) || player.isDead || player.getHealth() <= Float.intBitsToFloat(Float.floatToIntBits(1.0582841E38f) ^ 2124364703)) continue;
                for (Entity entity : new ArrayList<E>(ModuleAutoCrystal.mc.world.loadedEntityList)) {
            ...
            if (this.targetCrystal == null) break block18;
            swordSlot = InventoryUtils.findItem(Items.DIAMOND_SWORD, 0, 9);
            appleSlot = InventoryUtils.findItem(Items.GOLDEN_APPLE, 0, 9);
            if (!ModuleAutoCrystal.antiWeakness.getValue() || !ModuleAutoCrystal.mc.player.isPotionActive(MobEffects.WEAKNESS)) ** GOTO lbl-1000
            if (swordSlot != -1) {
